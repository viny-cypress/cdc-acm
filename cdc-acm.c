			    ...
			    unsigned int set, unsigned int clear)
{
    ...
    printk (KERN_ERR,"Calling TIOCMSET %x %x\n", set, clear);
						...
						struct ktermios *termios_old)
{
    ...
    if (termios->c_cflag & CRTSCTS){
        acm_ctrl_msg (acm, 0x60, 0x03, NULL, 0);
    }
    else
        acm_ctrl_msg (acm, 0x60, 0x00, NULL, 0);
	...
	if (!newline.dwDTERate) {
		...
		newctrl |=  ACM_CTRL_DTR;
	printk ("The baud rate that is being set is %d %x\n",newline.dwDTERate, newctrl);
